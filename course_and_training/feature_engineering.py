...
le.inverse_transform([0, 1])        # label encode ettiğimizde hangi değer neye karşılık geliyordu onu görmek için inverse transform u kullanıyoruz
...
df.shape
df["EMERGENCYSTATE_MODE"].nunique()
len(df["EMERGENCYSTATE_MODE"].unique())
binary_cols = [col for col in df.columns if df[col].dtype not in [int, float]
               and df[col].nunique() == 2]

## binary_cols = [col for col in df.columns if df[col].dtype not in [int, float] and df[col].nunique() == 2 and len(df[col].unique()) == 2]
## eksik değerlere de label encode yapmasın diye
# len(binary_cols)
...
pd.get_dummies(df, columns=["Embarked"], dummy_na=True).head()       ## eksik değerler de bir değişken olarak gelsin
